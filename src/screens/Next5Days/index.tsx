...
import { IForecastData } from '../Search/interfaces';

export type Next5DaysScreenNavigationProp = NativeStackNavigationProp<IStackRoutes, 'Next5Days'>;
...
const Next5Days = ({ navigation, route: { params } }: Props): JSX.Element => {
	...
	const [filteredDaysForecast, setFilteredDaysForecast] = useState<Array<IForecastDay>>([]);

	const isAndroid = Platform.OS === 'android';

	const filterForecastDays = () => {
		let filteredList: React.SetStateAction<IForecastDay[]> = [];
		let previousDate = '';
					...
					<Styled.Container>
						...
						<Styled.PaddingHorizontal>
							<HeaderNavigation
								...
								icon={<MaterialIcons name="calendar-today" size={18} color="white" />}
							/>

							<Divider top={30} />

							<Styled.ContainerSummaryTemperature>
								<Styled.Image source={forecastConditionsIcons(firstLetterUpperCase(weather[0].description))} />
